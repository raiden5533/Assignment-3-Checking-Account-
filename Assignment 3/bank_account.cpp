...
void bank_account::printBalance(bank_account yourBank[], int i) {

	std::cout << std::setfill('-') << std::setw(80) << "-" << std::endl
		<< "Customer Name:   " << yourBank[i].customerName << std::endl
		<< "Account Balance: " << std::fixed << std::setprecision(2) << yourBank[i].balance << std::endl << std::endl;
	return;
}
...
int bank_account::getNumOfAccounts(bank_account &yourBank) {
	return createdAcc;
}
